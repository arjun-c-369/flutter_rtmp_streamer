            ...
            child: Center(
              child: Container(
                decoration: const BoxDecoration(
                    color: Colors.white30, borderRadius: BorderRadius.all(Radius.circular(20))),
    ...
    return StreamBuilder<StreamingState>(
        ...
        builder: (context, streamingState) {
          return ElevatedButton(
              style: streamingState.data?.isStreaming ?? false
                ...
                try {
                  if (streamer.state.isStreaming) {
                    ...
                    streamer.startStream(
                        uri: "rtmp://flutter-webrtc.kuzalex.com/live", streamName: "one"
                        // )
                        );
                  }
                } catch (e) {
              ...
              child: Text(streamingState.data!.isStreaming ? "Stop streaming" : "Start streaming"));
        });
  }
}
...
class Loader extends StatelessWidget {
  ...
  const Loader({Key? key, this.radius = mediumRadius, this.brightness = Brightness.light})
      : super(key: key);
